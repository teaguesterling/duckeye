...
duckeye -d -Q ".call#eval" app.js    # Query AST call sites with line numbers & peek text
...
* **Universal Data Inspection**: Tabular data, configs, and archives viewable with one tool (`-d`), with built-in summarization (`-z`) and sparkline profiling (`-Z`).
